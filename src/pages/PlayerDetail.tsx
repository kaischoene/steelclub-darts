  ...
  return (
    <AppShell hasBottomBar>
      ...
      <div className="relative">
        ...
        <div className="relative flex items-center justify-between p-4 pt-6">
          <button
            ...
            className="w-10 h-10 rounded-full bg-white shadow-soft border border-black/[0.06] flex items-center justify-center active:scale-95"
          >
            <ChevronLeft size={20} className="text-ink" strokeWidth={2.5} />
          </button>
          <button className="w-10 h-10 rounded-full bg-white shadow-soft border border-black/[0.06] flex items-center justify-center active:scale-95">
      ...
      <section className="px-4 mt-6">
        <div className="bg-white border border-black/[0.06] rounded-2xl p-5 shadow-soft space-y-3">
          {player.hand && (
        ...
        <section className="mt-6 px-4">
          ...
          <div className="flex gap-3 overflow-x-auto scroll-hide snap-x pb-2 -mx-4 px-4">
            {upcomingEvents.map((e) => (
              <Link key={e.id} to={`/events/${e.slug}`} className="w-[180px] flex-shrink-0 snap-start bg-white border border-black/[0.06] rounded-2xl shadow-soft hover:shadow-card hover:-translate-y-1 transition-all p-4">
      ...
      <section className="px-4 mt-6 flex justify-center gap-3">
        <button className="w-12 h-12 rounded-full bg-white border border-black/[0.06] shadow-soft flex items-center justify-center hover:bg-orange hover:text-white hover:border-orange transition-all active:scale-95" aria-label="Facebook">
        ...
        <button className="w-12 h-12 rounded-full bg-white border border-black/[0.06] shadow-soft flex items-center justify-center hover:bg-orange hover:text-white hover:border-orange transition-all active:scale-95" aria-label="Instagram">
        ...
        <button className="w-12 h-12 rounded-full bg-white border border-black/[0.06] shadow-soft flex items-center justify-center hover:bg-orange hover:text-white hover:border-orange transition-all active:scale-95" aria-label="Website">
          <Globe size={20} />
        </button>
      </section>
...
function StatCard({ label, value, icon }: { label: string; value: string | number; icon: React.ReactNode }) {
  return (
    <div className="bg-white border border-black/[0.06] rounded-2xl p-4 shadow-soft">
